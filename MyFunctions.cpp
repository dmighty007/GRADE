...
vector<vector<double>> calc_F4_all(int count_solvent, int count_solute, vector<vector<int>>& My_neigh, vector<vector<double>>& atom_Pos, double& boxX, double& boxY, double& boxZ, vector<int>& Nneigh, int Natoms, int topSolute, string time, double HBOND_DIST ){
    
    vector<int> currentPair;        //Current pair of axygen atoms to be considered.
    vector<double> temp_vec;        //This is used to sort the 4 H pair distances.
    vector<double> A, B, C, D, AB, BC, CD, ABC, BCD;
    vector<vector<double>> each_f4;
    map<vector<int>,int> pairMap;   //A map to exclude double calculation of torsion angle between atom 'i-j' or 'j-i'.
    double dx=0, dy=0, dz=0 ;       //Distances for each pair.
    double dist_OA_H1B, dist_OA_H2B, dist_OB_H1A, dist_OB_H2A;      //Total distance for each pair.
    double phi=0, phi_avg=0, sum=0, magnitude_ABC=0, magnitude_BCD=0, each_avg=0;          //Parameters used in cross product calculation.
    int dih_A_index = 0, dih_B_index = 0, dih_C_index = 0, dih_D_index = 0;         //Four atoms of the dihedral.
...
void print_usage()
{
    ...
    cout << "-[no]af4\t(yes)\t(AllF4.xvg)\n\tCompute four-body order parameter F4=< cos3𝝓 > for each solvent\n" ;
    cout << "-[no]cage\t(yes)\t\n\t Specify if want to calculate cages also.\n" ;
